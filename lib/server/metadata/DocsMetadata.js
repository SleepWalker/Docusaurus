...
class DocsMetadata {
  ...
  populate() {
    ...
    const sidebarIndex = this.getSidebarIndex();
   ...
   */
  getSidebarIndex() {
    if (!this.sidebarIndex) {
      this.sidebarIndex = createDocsIndex({
        ...this.sidebars,
   ...
   * }
   */
  resolveSidebarMetadata(metadata, sidebarItem) {
    const {translation} = this;

    const sidebarData = {};
    sidebarData.sidebar = sidebarItem.sidebar;
    sidebarData.category = sidebarItem.category;

    ['next', 'previous'].forEach(key => {
      if (sidebarItem[key]) {
        sidebarData[`${key}_id`] = sidebarItem[key].replace(
