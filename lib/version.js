...
const {resolveSidebarItem} = require('./server/metadata/createDocsIndex.js');
...
if (versionFallback.diffLatestSidebar()) {
  ...
  Object.keys(sidebar).forEach(sb => {
    ...
    Object.keys(categories).forEach(category => {
      ...
      ids.forEach(id => {
        const sidebarItem = resolveSidebarItem(id, sidebar);
        const prefix = sidebarItem.prefix ? sidebarItem.prefix + '.' : '';

        versioned[versionSidebar][category].push(
          `${prefix}version-${version}-${sidebarItem.id}`
        );
      });
    });
  });
